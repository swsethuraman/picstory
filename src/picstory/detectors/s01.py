"""S01 · Human in the foreground — Anthropic API vision-call detector.

Judgment-dependent per QUEUE.md item 4 and CLAUDE.md's API-discipline rule:
the call embeds S01's Detection text verbatim (via
`schema.taxonomy_detection_text`, not a local copy) and returns structured
output naming S01. See `_vision.py` for the shared call/parse plumbing.
"""
