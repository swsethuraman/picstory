...
Item counts: 15 failure modes, 4 strengths, 1 conditional rule, 1 comparison rubric.

---

## How the taxonomy feeds the outputs

| Output | Draws on |
|---|---|
| **The pick** (and the share list) | Strengths (S-items) as the "why it's share-worthy" one-liners; failure modes as disqualifiers |
| **The habit** (one per session) | Whichever F- or S-item recurs most in the batch; reinforcement counts as coaching |
| **Three-frame comparison** | The CMP rubric, exclusively |
| **The running profile** | Per-user recurrence of F/S items and their sub-patterns (e.g. *which* edge the user neglects) |
...
### F04 · Ultrawide distortion on people
- **Polarity:** −
...
### F05 · Ultrawide geometric distortion
- **Polarity:** −
...
### F06 · Edge intrusion (unchosen frame content)
- **Polarity:** −
...
### F11 · Busy background behind figures
- **Polarity:** −
...
### F15 · Subject/landmark competition
- **Polarity:** −
...
## S — Strengths (reinforcement items)

Coaching includes telling the user what to keep doing. These also supply the share-list one-liners.

### S01 · Human in the foreground
- **Polarity:** +
- **Detection:** A person meaningfully placed in the foreground of a scene or landmark shot.
- **Reinforcement:** These consistently outrank empty landmark shots — keep leading with people.
- **Example:** The night portraits.

### S02 · Blue-hour timing
- **Polarity:** +
- **Detection:** Shot in the post-sunset / pre-dark window; deep blue sky with lit subjects.
- **Reinforcement:** Repeatedly a strength — plan key locations for this window.

### S03 · Tight framing
- **Polarity:** +
- **Detection:** The tightest frame of a subject among its batch-mates.
- **Reinforcement:** The tightest frames won every batch in the source sessions.

### S04 · Restraint composition
- **Polarity:** +
- **Detection:** A single saturated or high-interest object against a quiet, uncluttered field.
- **Reinforcement:** Produced the trip's best still-life frame (the red vessel in the white niche).

---

## R — Conditional rules

Rules are triggered by shooting conditions, not detected in frames. Different object type for the classifier: they generate *forward-looking* advice.

### R01 · Haze rule
- **Trigger:** Hazy / low-contrast conditions (detected via F12 findings in the batch).
- **Rule:** Shoot tighter. Tight frames suppress haze's flattening effect; the tightest frames consistently ranked highest.

---

## CMP — The three-frame comparison rubric

The scoring axes for near-duplicate comparison (Stage 3). Promoted from the draft inventory's B4: this is the rubric for the product's core feature, not a failure mode. The failure of *taking* undifferentiated duplicates is F03; this is how the app *judges between* them.

When comparing near-identical frames, score and explain on exactly three axes:

1. **Subject placement** — position relative to center / thirds / the composition's focal lines.
2. **Edge amputations** — which elements each frame cuts at its edges, and whether the cuts are committed or accidental.
3. **Incidental distractions** — exit signs, vehicles, bystanders, clutter present in one frame and absent in another.

**Tiebreaker:** a story element wins. A frame with a mid-stride walker or a gaze line beats a cleaner empty record of the same scene.

The comparison output names the winning frame and states, per axis, what differs — in terms the user can change on purpose next time (which is how the rubric loops back into coaching).

---

## U — Unclassified observations (the expansion pipeline)

The taxonomy is closed at runtime and amended only by versioned human edit. The bridge between those two facts is this bucket.

**Runtime behavior:** when the analysis finds something worth saying that maps to no F/S/R item, it does not force-fit it to the nearest item and does not invent a new category. It emits the observation as `unclassified`, with a free-text description, and logs it.

**Schema:** every per-frame finding in the analysis output carries either a taxonomy ID or `unclassified` + description. One field. That is the entire v1 build cost of this section.

**Expansion process:** unclassified logs are reviewed periodically by a human. An observation graduates to a taxonomy item when it recurs — across sessions for one user, or across users — and a versioned edit adds it with a changelog entry. IDs are stable and never reused, so amendments never break historical profiles.

**What expansion pressure to expect:**
- *Per-user variation mostly is not expansion.* Different users' habits are overwhelmingly frequency and sub-pattern within existing items (which edge, which conditions) — profile-layer data, not new taxonomy items.
- *Domain variation is the real first wave.* The source data is entirely travel/architecture; food, pets, kids' sports, and concerts will surface genuinely absent modes (e.g. motion blur on moving subjects — nothing in the source material moved). Expect the first additions to be domain-shaped, not user-shaped.

**Standing rule for agents:** adding, removing, or editing taxonomy items is never a proceed-without-asking action. Log and skip. (Mirrored in `CLAUDE.md`.)
