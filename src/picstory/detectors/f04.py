"""F04 · Ultrawide distortion on people — Anthropic API vision-call detector.

Judgment-dependent per QUEUE.md item 4 and CLAUDE.md's API-discipline rule:
the call embeds F04's Detection text verbatim (via
`schema.taxonomy_detection_text`, not a local copy) and returns structured
output naming F04. See `_vision.py` for the shared call/parse plumbing.
"""
